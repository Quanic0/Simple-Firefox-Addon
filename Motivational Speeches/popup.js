...
function displaySpeech(number) {
  ...
  const speech = speeches[number - 1] || "No motivational speech found for this number.";
  document.getElementById('speech').textContent = speech;
}

function getAndDisplaySpeech() {
  const randomNumber = getRandomNumber();
  displaySpeech(randomNumber);
}

document.addEventListener('DOMContentLoaded', function() {
  getAndDisplaySpeech(); // Get and display speech when popup is opened initially
  setInterval(getAndDisplaySpeech, 3600000); // Get and display speech every hour (in milliseconds)
});
